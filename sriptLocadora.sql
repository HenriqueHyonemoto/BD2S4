...
-------Produto Cartesiano (NÃO É UTILIZADO PARA CONSULTAS(USAR JUNÇÃO))--
select nome from ator, atorEstrelaFilme; 

-------Junção (Sempre igualar Chave Estrangeira com Chave Primaria)--

select nome, a.idAtor, codFilme from ator a, atorEstrelaFilme aef --Em Select, quando h[á consulta com mais de duas tabela, especificar de qual tabela é o atributo (a.idAtor ou AEF.idAtor)]
WHERE a.idAtor = AEF.idAtor; --(Chave Estrangeira = Chave Primaria (Ordem não Importa))
 

-------Junção Inner Join (Igual ate o primeiro atributo de FROM (select nome, a.idAtor, codFilme from ator a) Em vez das tabelas separadas por virgula, é utilizado o INNER JOIN)
...
--Nome dos atores e dos filmes que fizeram{
...
where f.nome like '%a%'; --Todos os filmes que possuem letra a, LIKE (Case Sensitive) ILIKE (Não é Case Sensitive)

--Com (WHERE e AND)
select a.nome, f.nome from ator a, atorEstrelaFilme aef, filme f
where a.idAtor = aef.idAtor and f.codFIlme = aef.codFIlme --;
and f.nome like '%a%'; --Todos os filmes que possuem letra a, LIKE (Case Sensitive) ILIKE (Não é Case Sensitive)

--}

--Selecionar clientes e telefones{
...
--Where e And
select c.nome, t.telefone from cliente c, telefoneCliente t
where c.codCliente = t.codCliente;
--}

--Nome dos atores que fizeram filmes de genero e drama{

--Com Inner Join
select a.nome ,f.nome, f.genero from ator a
inner join atorEstrelaFilme aef
on a.idAtor = aef.idAtor
inner join filme f
on f.codFilme = aef.codFilme
...
select a.nome, f.nome, f.genero from ator a, atorEstrelaFilme aef, filme f
where a.idAtor = aef.idAtor and f.codFilme = aef.codFilme and f.genero = 'Drama';


--}


--COnsulta que retorne os nomes e os generos dos filmes de ação ou aventura{
...
--Usando In
select nome,genero from filme
where genero in ('Ação','Aventura')
order by genero;

--}

--Nome dos atores e das datas de nascimento de cada ator que a data de nascimento seja entre 10/10/1970 e 10/10/1990.{
select nome, dtaNasc from ator 
where dtaNasc between '10/10/1970' and '10/10/1990'; -- Datas vão entre aspas simples 

select nome, dtaNasc from ator 
where dtaNasc > '10/10/1970' and dtaNasc < '10/10/1990'; -- DAtas vão entre aspas simples 

--}

--Consulta que retorne os generos e a quantidade de filmes que cada genero possui (GROUP BY){
select genero, count(nome) as "Quantidade de Filmes" from filme
group by genero;

select genero, count(genero) as "Quantidade de Filmes" from filme
group by genero having count(genero) > 3
order by genero;
